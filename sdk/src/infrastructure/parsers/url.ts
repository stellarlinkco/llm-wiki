...
let resolveHostAddresses: HostResolver = async (hostname) => (await lookup(hostname, { all: true })).map((entry) => entry.address);
...
function contentTypeSource(response: { headers: Record<string, string | string[] | number | undefined> }): { contentType?: string } {
...
function decodeText(bytes: Buffer, contentType: string | undefined): string {
