...
const ROOT_AREAS = ["sources", "concepts", "references"] as const;

export function buildProgressiveIndexFiles(
  entries: IndexCatalogEntry[],
  options: IndexCatalogOptions,
): Map<string, string> {
...
function buildDirectoryIndexes(files: Map<string, string>, directory: string, entries: IndexCatalogEntry[]): void {
  ...
  if (subdirectories.length > 0) {
    ...
    for (const subdirectory of subdirectories) {
      ...
      lines.push(
        `- [${capitalizeArea(subdirectory)}](${subdirectory}/index.md) — ${documentCountLabel(childEntries.length)}`,
      );
...
function listImmediateSubdirectories(directory: string, entries: IndexCatalogEntry[]): string[] {
...
function listImmediateDocuments(directory: string, entries: IndexCatalogEntry[]): IndexCatalogEntry[] {
  const prefix = `${directory}/`;
...
function countImmediateSubdirectories(directory: string, entries: IndexCatalogEntry[]): number {
...
function capitalizeArea(value: string): string {
  if (value === "") {
    return value;
  }
