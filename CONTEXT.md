...
|------|---------|-----------------|----------------|
| **OKF** (Open Knowledge Format) | The file-based format for knowledge bundles — Markdown pages with YAML frontmatter, organized under `sources/`, `concepts/`, `references/`, with `.llm-wiki/` metadata and `index.md` catalog | Not a database, not a vector store, not a graph DB | SDK contract `sdk/src/domain/types.ts` |
